...
import br.com.tanamao.dal.GetSetsItensOS;
...
public final class HistoricoOS extends javax.swing.JFrame {
...
//        TableCustom.apply(jScrollPane2, TableCustom.TableType.DEFAULT);
        ...
        tableOS.getTableHeader().setResizingAllowed(false);
...
//    }
//
    private void colocarDadosOS() {
        if (empresa.getText().equals("tanamao")) {
            String sql = "select idos as ID,numeroos as OS,nome as Nome,cpf as CPForCNPJ,DATE_FORMAT (`entrada`,'%d/%m/%Y') as Entrada,vendedor as Vendedor,DATE_FORMAT (`validade`,'%d/%m/%Y') as ValidadeOS,equipamento as Equipamento,total as TotalOS from os where empresa = 'tanamao' order by entrada";
            try {
                pst = conexao.prepareStatement(sql);

                rs = pst.executeQuery();
                tableOS.setModel(DbUtils.resultSetToTableModel(rs));
                tableOS.getColumnModel().getColumn(0).setWidth(90);
        ...
        if (empresa.getText().equals("docatec")) {
            String sql = "select idos as ID,numeroos as OS,nome as Nome,cpf as CPForCNPJ,DATE_FORMAT (`entrada`,'%d/%m/%Y') as Entrada,vendedor as Vendedor,DATE_FORMAT (`validade`,'%d/%m/%Y') as ValidadeOS,equipamento as Equipamento,total as TotalOS from os where empresa = 'docatec' order by entrada";
            try {
                pst = conexao.prepareStatement(sql);
    ...
    private void initComponents() {
        ...
        jLabel15.setIcon(new javax.swing.ImageIcon(getClass().getResource("/src/ordem-de-servico (2).png"))); // NOI18N
        jLabel15.setText("    HISTÓRICO DE ORDEM DE SERVIÇO");
        ...
        vendaconcluida.setText("OS em aberto");
        ...
        vendacancelada.setText("OS Canceladas");
        ...
        jPanel1Layout.setVerticalGroup(
            ...
            .addGroup(jPanel1Layout.createSequentialGroup()
                ...
                .addGroup(jPanel1Layout.createParallelGroup(javax.swing.GroupLayout.Alignment.BASELINE)
                    ...
                    .addComponent(vendatotal))
                .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.UNRELATED)
                .addComponent(tableScrollButton2, javax.swing.GroupLayout.DEFAULT_SIZE, 436, Short.MAX_VALUE)
                .addGap(18, 18, 18)
                .addGroup(jPanel1Layout.createParallelGroup(javax.swing.GroupLayout.Alignment.BASELINE)
                    .addComponent(jButton1)
                    .addComponent(jButton2)
...
//        }
    ...
    private void jButton2ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton2ActionPerformed
        ...
        int setarVendas = tableOS.getSelectedRow();
        if (setarVendas >= 0) {
            String Name = (tableOS.getModel().getValueAt(setarVendas, 2).toString());
            ...
            if (option == 1) {
                ...
                if (confirma == JOptionPane.YES_OPTION) {
                    try {
                        String sql;
                        sql = "select os.idos as ID,idcliente as IDCLI,nome as Nome,cpf as CPF,tel as TEL,DATE_FORMAT (`entrada`,'%d/%m/%Y') as Entrada,vendedor as Vendedor,chegada as CanalVenda,obs as OBS,validade as Validade,status as statusos,recebimento as receb,obsfrete as ObsFrete,endereco as Endec,entregador as Entregador,precofrete as PrecoF,dataentrega as DataEnt,pagador as Pagador,formapg as Formapg,parcelas as Parc,desconto as descont,valordesconto as valordesc,equipamento as equip,marca as Marca,modelo as Modelo,serial as Serialprod,config as config,itens as itenscliente,relato as relato,laudo as laudo,aparencia as apar,causas as causas,sugestao as sugestao,total as total,numeroos as identifier,itemos.idproduto as ID,itemos.descricaoprod as Descricao,itemos.serialprod as Serial,itemos.marcaprod as Marca,itemos.precoprod as PrecoProd,itemos.qtdprod as QtdeProd,itemos.subtotal as SUB,servicoos.servico as Servico,servicoos.idservico,servicoos.qtde as qtdeserv,servicoos.preco as Precoserv,servicoos.subtotal as subtotalserv from os \n"
                                + "JOIN itemos on os.idos = itemos.idos \n"
                                + "JOIN servicoos on os.idos = servicoos.idos\n"
                                + "where os.idos = ?;";
                        pst = conexao.prepareStatement(sql);
                        pst.setString(1, tableOS.getModel().getValueAt(setarVendas, 0).toString());
                        rs = pst.executeQuery();
                        InputStream report = getClass().getResourceAsStream("/recibos/os.jasper");

                        String os = System.getProperty("os.name");
                        if (os.startsWith("Windows")) {
                            List <GetSetsItensOS> listItens = new ArrayList<GetSetsItensOS>();
                            GetSetsItensOS gt1 = new GetSetsItensOS();
                            GetSetsItensOS gt2 = new GetSetsItensOS();
                            
                            gt1.setDescricao("teste");
                            gt1.setPreco("10");
                            gt1.setQtd("1");
                            gt1.setSerial("tasdaasd");
                            listItens.add(gt1);
                            
                            
                            JRBeanCollectionDataSource itemjrBean = new JRBeanCollectionDataSource(listItens);
                            Map<String, Object> parameters = new HashMap<String, Object>();
                            parameters.put("jrbeanparam", itemjrBean);
                            
                            
                            
                            JRResultSetDataSource relatResul = new JRResultSetDataSource(rs);
                            JasperPrint jasperPrint = JasperFillManager.fillReport(report, parameters, relatResul);
                            JasperExportManager.exportReportToPdfFile(jasperPrint, "C://convtec/Supply-x/trash/tempreciboOS.pdf");
                            File file = new File("C://convtec/Supply-x/trash/tempreciboOS.pdf");
...
//                        try {
//                            String sql;
//                            sql = "select idvendas as N_VENDA,idcliente as IDCLI,nomecli as Nome,cpfcli as CPF,telcli as TEL,DATE_FORMAT (`datacompra`,'%d/%m/%Y') as DataCompra,vendedor as Vendedor,canalvenda as CanalVenda,DATE_FORMAT (`garantia`,'%d/%m/%Y') as Garantia,formapagamento as FormaPG,valorcdesconto as Valorc,total as Total,status as Status,itemvenda.idproduto as ID,itemvenda.descricaoprod as Descricao,nparcelas as Parc,itemvenda.serialprod as Serial,itemvenda.marcaprod as Marca,itemvenda.precoprod as Preco,itemvenda.qtdeprod as Qtde,itemvenda.subtotal as SUB from vendas INNER JOIN itemvenda on vendas.idvendas = itemvenda.idvenda where idvendas=?";
//                            pst = conexao.prepareStatement(sql);
//                            pst.setString(1, tablevendas.getModel().getValueAt(setarVendas, 0).toString());
//                            rs = pst.executeQuery();
...
//        }
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                new HistoricoOS().setVisible(true);
            }
        });
    }
